function Img_F_norm = Normalize_DFoF0( handles )
...
% input dialog for checking the 
 prompt = {'Type the number of initial frames, at least 3, for experiments of evoked activity; alternatively type -1 to use all available frames.'};
 dlg_title = 'Resting fluorescence (F0) estimation';
 num_lines = 1;
 defaultans = {'-1'};
 answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
 nframes = str2double( answer );
...
if nframes == -1
 ...
 F0 = median( handles.Img(:,:,1:nframes), 3 );
