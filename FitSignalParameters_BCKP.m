...
% a*exp(-t/t1)+b*exp(-t/t2)+c
    ...
    %% fit single exp 
    %fit_opt = fitoptions( 'Method', 'NonlinearLeastSquares', 'Algorithm', 'Trust-Region', 'Lower', [0, 0, -Inf], 'Upper', [Inf, Inf, Inf], 'StartPoint', [1.0, 1000.0, 1000.0] );
    fit_opt = fitoptions( 'Method', 'NonlinearLeastSquares', 'Algorithm', 'Trust-Region', 'Lower', [0, -Inf, -Inf, -Inf], 'Upper', [Inf, Inf, Inf, Inf], 'StartPoint', [1.0, 0.0, 1000.0, 1000.0] );
    %fit_fun = fittype( 'A * exp(-t/t_decay) + C0', 'independent', 't', 'options', fit_opt);
    fit_fun = fittype( 'A * exp(-(t-t0)/t_decay) + C0', 'independent', 't', 'options', fit_opt);

    %% fit single exp - decay
    %fit_opt = fitoptions( 'Method', 'NonlinearLeastSquares', 'Algorithm', 'Trust-Region', 'Lower', [0, 0, -Inf], 'Upper', [Inf, Inf, Inf], 'StartPoint', [1.0, 1000.0, 1000.0] );
    fit_opt = fitoptions( 'Method', 'NonlinearLeastSquares', 'Algorithm', 'Trust-Region', 'Lower', [0, -Inf, -Inf, -Inf], 'Upper', [Inf, Inf, Inf, Inf], 'StartPoint', [1.0, 0.0, 1000.0, 1000.0] );
    %fit_fun = fittype( 'A * exp(-t/t_decay) + C0', 'independent', 't', 'options', fit_opt);
    fit_fun = fittype( 'A * exp(-(t-t0)/t_decay) + C0', 'independent', 't', 'options', fit_opt);
